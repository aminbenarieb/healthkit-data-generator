...
public struct ChatContentView: View {
    ...
    public var body: some View {
        NavigationStack {
            ...
            .navigationTitle("Health Generator")
            ...
            .alert(item: $healthManager.generationResult) { result in
                switch result {
            ...
            .task {
                if HKHealthStore.isHealthDataAvailable() && !healthManager.isAuthorized {
    ...
    private var authorizationSection: some View {
        VStack(spacing: 12) {
            HStack {
                ...
                Text(healthManager.isAuthorized ? "Health Authorized" : "Health Authorization Required")
                    .font(.headline)
    ...
    private var generateButton: some View {
        VStack(spacing: 12) {
            ...
            .disabled(healthManager.isGeneratingInProgress || !healthManager.isAuthorized)
            
            if healthManager.isGeneratingInProgress {
                HStack {
                    ProgressView()
                        .scaleEffect(0.8)
    ...
    private var cleanDataButton: some View {
        VStack(spacing: 12) {
            Button(action: {
                healthManager.cleanHealthData()
            }) {
                HStack {
                    if healthManager.isCleaningInProgress {
                        ProgressView()
                            .progressViewStyle(CircularProgressViewStyle(tint: .white))
                            .scaleEffect(0.8)
                    } else {
                    ...
                    Text(healthManager.isCleaningInProgress ? "Cleaning..." : "Clean Health Data")
                        .font(.headline)
                        .fontWeight(.semibold)
                }
                .foregroundColor(.white)
                ...
                .background(
                    RoundedRectangle(cornerRadius: 12)
                        .fill(healthManager.isCleaningInProgress ? Color.gray : Color.red)
                )
            }
            .disabled(healthManager.isCleaningInProgress)
            ...
            if healthManager.isCleaningInProgress {
                VStack(alignment: .leading, spacing: 8) {
                    ...
                    if let progress = healthManager.cleaningProgress {
                    ...
                    Text(healthManager.cleaningMessage)
                        .font(.caption)
                        .foregroundColor(.secondary)
                }
                .padding()
    ...
    private func sendMessage() {
        ...
        Task {
            do {
                let response = try await healthManager.generateWithLLM(prompt: message)
                ...
                await MainActor.run {
                    let errorMessage = ChatMessage(content: "Sorry, I couldn't process your request. Please try again. Reason: \(error.localizedDescription)", isUser: false)
    ...
    private func generateHealthDataWithEnhancedConfig() {
        ...
        healthManager.generateHealthData(config: config)
    }
}
