...
/// Logging wrapper for HealthDataGenerator package using swift-log
/// Follows Apple's best practices for structured logging
public enum AppLogger {
    
    // MARK: - Package Loggers (SPM)
    
    private static let packageSubsystem = "com.aminbenarieb.health-data-generator.package"
    ...
    /// Logger for Health store operations
...
public enum AppUILogger {
    ...
    private static let appSubsystem = "com.aminbenarieb.health-data-generator.app"
    ...
    /// Logger for Health authorization
...
extension Logger {
    ...
    /// Log Health save
    public func logHealthSave(sampleType: String, success: Bool) {
        if success {
            self.debug("💾 Saved to Health store", metadata: ["type": "\(sampleType)"])
        } else {
