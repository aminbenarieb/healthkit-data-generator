...
public protocol SampleCreator {
    ...
    /// - Returns: a Health sample that can be saved to the Health store or nil.
...
extension SampleCreator {
    ...
    func dictToTimeframe(_ dict:Dictionary<String, AnyObject>) -> (sDate:Date, eDate:Date) {
		...
		let startDate: Date
		if let timestamp = dict[HealthConstants.S_DATE] as? Double {
			startDate = Date(timeIntervalSince1970: timestamp / 1000)
		} else if let stringDate = dict[HealthConstants.S_DATE] as? String {
		...
		let endDate: Date
		if let timestamp = dict[HealthConstants.E_DATE] as? Double {
			endDate = Date(timeIntervalSince1970: timestamp / 1000)
		} else if let stringDate = dict[HealthConstants.E_DATE] as? String {
    ...
    func dictToCategorySample(_ dict:Dictionary<String, AnyObject>, forType type: HKCategoryType) -> HKCategorySample {
        let value = dict[HealthConstants.VALUE] as? Int ?? 0
    ...
    func dictToQuantitySample(_ dict:Dictionary<String, AnyObject>, forType type: HKQuantityType) -> HKQuantitySample {
        ...
        let value   = dict[HealthConstants.VALUE] as! Double
        let strUnit = dict[HealthConstants.UNIT] as? String
...
class CorrelationSampleCreator : SampleCreator {
    ...
    func createSample(_ sampleDict: AnyObject) -> HKSample? {
        ...
        if let dict = sampleDict as? Dictionary<String, AnyObject> {
            ...
            if let objectsArray = dict[HealthConstants.OBJECTS] as? [AnyObject] {
                for object in objectsArray {
                    if let subDict = object as? Dictionary<String, AnyObject> {
                        let subTypeName = subDict[HealthConstants.TYPE] as? String
...
class WorkoutSampleCreator : SampleCreator {
    ...
    func createSample(_ sampleDict: AnyObject) -> HKSample? {
        ...
        if let dict = sampleDict as? Dictionary<String, AnyObject>,
            let activityTypeRawValue = dict[HealthConstants.WORKOUT_ACTIVITY_TYPE] as? UInt,
            let activityType = HKWorkoutActivityType(rawValue: activityTypeRawValue) {
            let dates = dictToTimeframe(dict)

            let duration = dict[HealthConstants.DURATION] as? TimeInterval
            let totalDistance = dict[HealthConstants.TOTAL_DISTANCE] as? Double // always HKUnit.meterUnit()
            let totalEnergyBurned = dict[HealthConstants.TOTAL_ENERGY_BURNED] as? Double //always HKUnit.kilocalorieUnit()

            var events:[HKWorkoutEvent] = []

            if let workoutEventsArray = dict[HealthConstants.WORKOUT_EVENTS] as? [AnyObject] {
                for workoutEvent in workoutEventsArray {
                    if let subDict = workoutEvent as? Dictionary<String, AnyObject> {
                        let eventTypeRaw = subDict[HealthConstants.TYPE] as? Int
