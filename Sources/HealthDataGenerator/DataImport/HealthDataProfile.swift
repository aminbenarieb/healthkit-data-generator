...
open class HealthDataProfileMetaData {
...
// MARK: - Health Data Profile

/// A health data Profile - can be used to read data from the profile and import the profile into the health store.
open class HealthDataProfile : CustomStringConvertible {
    ...
    public init(healthStore: HKHealthStore, fileAtPath: URL) throws {
    ...
    /// - Returns: the HealthDataProfileMetaData that were read from the profile.
    internal func loadMetaData() -> HealthDataProfileMetaData {
        let result          = HealthDataProfileMetaData()
    ...
    open func loadMetaData(_ asynchronous:Bool, callback:@escaping (_ metaData: HealthDataProfileMetaData) -> Void ){
