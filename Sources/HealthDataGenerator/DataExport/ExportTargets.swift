...
// MARK: - JSON Single Document Export Target

/// An export target that generates a single json doc for the whole data.
open class JsonSingleDocExportTarget  {
    ...
    open func writeMetaData(creationDate: Date, profileName: String, version: String) {
        ...
        jsonWriter.writeObjectFieldStart(HealthConstants.META_DATA)

        jsonWriter.writeField(HealthConstants.CREATION_DATE, value: creationDate)
        jsonWriter.writeField(HealthConstants.PROFILE_NAME, value: profileName)
        jsonWriter.writeField(HealthConstants.VERSION, value: version)
        jsonWriter.writeField(HealthConstants.TYPE, value: String(describing: JsonSingleDocExportTarget.self))
    ...
    open func writeUserData(_ userData: Dictionary <String, AnyObject>) throws {
        try jsonWriter.writeFieldWithObject(HealthConstants.USER_DATA, value: userData)
    }
...
// MARK: - JSON Single Document As File Export Target
...
// MARK: - JSON Single Document In Memory Export Target
