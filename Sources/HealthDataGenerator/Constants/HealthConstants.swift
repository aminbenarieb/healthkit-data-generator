...
public class HealthConstants {
    ...
    static let healthCharacteristicsTypes: Set<HKCharacteristicType> = Set(arrayLiteral:
    ...
    static let healthCategoryTypes: Set<HKCategoryType> = Set(arrayLiteral:
...
//        HKObjectType.categoryType(forIdentifier: HKCategoryTypeIdentifier.appleStandHour)!
    ...
    // not writable
    static let healthCategoryLockedTypes: Set<HKCategoryType> = Set(arrayLiteral:
        HKObjectType.categoryType(forIdentifier: HKCategoryTypeIdentifier.appleStandHour)!
    )

    static let healthQuantityTypes: Set<HKQuantityType> = Set(arrayLiteral:
        // Body Measurements
...
//        HKObjectType.quantityType(forIdentifier: HKQuantityTypeIdentifier.walkingHeartRateAverage)!,
    ...
    //not writable
    static let healthQuantityLockedTypes: Set<HKQuantityType> = Set(arrayLiteral:
                HKObjectType.quantityType(forIdentifier: HKQuantityTypeIdentifier.nikeFuel)!
    )

    static let healthCorrelationTypes: Set<HKCorrelationType> = Set(arrayLiteral:
    ...
    static func allTypes() -> Set<HKObjectType> {
        ...
        allTypes.formUnion((healthCharacteristicsTypes as Set<HKObjectType>?)!)
        allTypes.formUnion((healthQuantityTypes as Set<HKObjectType>?)!)
        allTypes.formUnion((healthCategoryTypes as Set<HKObjectType>?)!)
        allTypes.formUnion((healthCorrelationTypes as Set<HKObjectType>?)!)
    ...
    public static func authorizationReadTypes() -> Set<HKObjectType> {
        ...
        authTypes.formUnion((HealthConstants.healthCharacteristicsTypes as Set<HKObjectType>?)!)
        authTypes.formUnion((HealthConstants.healthQuantityTypes as Set<HKObjectType>?)!)
        authTypes.formUnion((HealthConstants.healthQuantityLockedTypes as Set<HKObjectType>?)!)
        authTypes.formUnion((HealthConstants.healthCategoryTypes as Set<HKObjectType>?)!)
        authTypes.formUnion((HealthConstants.healthCategoryLockedTypes as Set<HKObjectType>?)!)
        authTypes.insert(HealthConstants.workoutType)
        return authTypes
    }

    public static func authorizationWriteTypes() -> Set<HKSampleType> {
         var authTypes : Set<HKSampleType> = Set()
        authTypes.formUnion((HealthConstants.healthQuantityTypes as Set<HKSampleType>?)!)
        authTypes.formUnion((HealthConstants.healthCategoryTypes as Set<HKSampleType>?)!)
        authTypes.insert(HealthConstants.workoutType)
        authTypes.insert(HealthConstants.heartbeatSeriesType)
        return authTypes
    }
