...
/// Main class for generating and populating Health data
final public class HealthDataGenerator {
    ...
    /// Generates and populates Health store with sample data using configuration
    /// - Parameters:
    ...
    /// Generates samples based on configuration without populating Health store
    ...
    /// Populates Health store with generated samples
    /// - Parameters:
    ///   - samplesTypes: Set of sample types to populate
    ...
    public func populate(samplesTypes: Set<HKSampleType>, generatedSamples: [String: Any]) throws {
        ...
        AppLogger.healthKit.info("Starting to populate Health store", metadata: [
    ...
    private func validateConfig(_ config: SampleGenerationConfig) throws {
        ...
        guard profile.sleepDurationRange.upperBound <= 12.0 else {
